...
|--------|-------------|---------|
| `--source <file>` | Path to import file (required). | - |
| `--locale <locale>` | Target locale for import (e.g., `es`, `fr-ca`). | - |
| `--format <format>` | Import format (`xliff` or `json`). | Auto-detect from extension |
| `--collection <name>` | Target collection to import into. | Default collection |
| `--strategy <strategy>` | Import strategy (see below). | `translation-service` |
| `--update-comments` | Update resource comments from import data. | Strategy-dependent |
| `--update-tags` | Update resource tags from rich JSON. | Strategy-dependent |
...
- Does NOT create new resources (skips with error if resource doesn't exist)
...
- Sets status to `translated` for all imports
- Expands Transloco key references (e.g., `{{t('other.key')}}`) into full values
- Updates comments and tags from import data by default
...
**Behavior**:
- Updates only existing resources (skips with warning if resource not found)
...
- Errors for unfixable cases (multiple missing placeholders, malformed syntax)
...
Every import generates an `import-summary.md` file with detailed change tracking:
...
The dry-run summary shows exactly what would happen, using "Would" language to indicate no changes were made.
...
- Source file selection (with file picker/autocomplete)
