...
### Phase 1: Core Infrastructure & Common Logic
...
- [ ] Implement `export-to-json.ts`:
  ...
  - `buildRichObject()` - Metadata-enriched objects
...
### Format Output
- [ ] XLIFF 1.2 export generates valid, schema-compliant files using `xliff` library
- [ ] XLIFF files can be opened in professional translation tools (SDL Trados, MemoQ, etc.)
- [ ] XLIFF output includes no custom status attributes or tags (standard XLIFF 1.2 only)
- [ ] JSON export supports all four format combinations (flat/hierarchical × simple/rich)
- [ ] JSON rich objects include only defined fields (omit undefined/nil values)
- [ ] ICU message format syntax is preserved as-is in both XLIFF and JSON

### Filtering & Selection
- [ ] Status filtering works on a per-locale basis
- [ ] Tag filtering includes resources with at least one matching tag
- [ ] Base locale is never exported (only target locales)
- [ ] Resources without metadata are omitted and logged as errors
- [ ] Multiple collections are merged into single output files per locale
- [ ] Duplicate keys across collections use last-write-wins strategy
- [ ] Empty filter results do not create files and log warnings

### CLI Interface
- [ ] CLI supports both interactive (TTY) and non-interactive (CI/CD) modes
- [ ] `--dry-run` mode reports what would be exported without writing files
- [ ] `--verbose` mode shows each resource being processed and each file written
- [ ] `--include-tags` flag controls tag export in JSON rich format
- [ ] `--tags` flag filters resources by tag
- [ ] `--filename` supports placeholders: `{target}`, `{source}`, `{locale}`, `{date}`
- [ ] Default filenames: `{target}.xliff` for XLIFF, `{locale}.json` for JSON

### Error Handling
- [ ] Malformed JSON files are skipped with detailed warnings
- [ ] Non-writable output directory fails early with clear error
- [ ] Hierarchical key conflicts are logged and reported in summary
- [ ] File overwrites generate warning messages
- [ ] Large exports use streaming with fallback to in-memory assembly

### Export Summary
- [ ] Every export generates `export-summary.md` in output directory
- [ ] Summary includes statistics: resources exported, files created, filters applied
- [ ] Summary lists all warnings: missing collections, overwritten files
- [ ] Summary lists all errors: malformed files, omitted resources, key conflicts
- [ ] Dry-run mode generates summary showing what would be exported

### Architecture & Code Quality
- [ ] All core business logic resides in `libs/core/src/lib/export/`
- [ ] Separate files for XLIFF, JSON, common logic, and summary generation
- [ ] Core functions return `ExportResult` with comprehensive error/warning tracking
- [ ] CLI command follows existing patterns (init, add-resource commands)
- [ ] Unit tests cover core export logic, filtering, and format generation
- [ ] Integration tests verify end-to-end export with sample data
- [ ] Architecture supports future pluggable export formats